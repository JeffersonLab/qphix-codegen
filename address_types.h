...
{
public:
    GaugeAddress(const string& base_, int dir_, int c1_
               , int c2_, int reim_, int isHalfType) 
    : Address(isHalfType), base(base_), dir(dir_), c1(c1_), c2(c2_), reim(reim_) 
    {};
...
{
public:
    ClovDiagAddress(const string& base_, int block_
                  , int component_, int isHalfType) 
    : Address(isHalfType), base(base_), block(block_), component(component_) {};
...
{
public:
    ClovOffDiagAddress(const string& base_, int block_
                     , int component_, int reim_, int isHalfType) 
    : Address(isHalfType), base(base_), block(block_), component(component_)
    , reim(reim_) {};
...
{
public:
    SpinorAddress(const string& base_, int spin_, int color_
                , int reim_, int isHalfType) 
    : Address(isHalfType), base(base_), spin(spin_), color(color_), reim(reim_) 
    {};
    string serialize(void) const
    {
        ostringstream outbuf;
...
public:
    ...
    : Address(a_->isHalfType()), a(a_), offset_var(offset_var_), imm(imm_) {};
    AddressType getType(void) const
    {
        return a->getType();
    }
...
public:
    ...
    : Address(base_->isHalfType()), base(base_), offsets(offsets_) {};
    string serialize(void) const
    {
        ostringstream outbuf;
